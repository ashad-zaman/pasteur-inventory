...
class Settings extends MY_Controller {
	...
	public function output($Return=array()){
     ...
     {	
		...
		$data = array(
			...
			'fax' => $company_info[0]->fax,
			'phone' => $company_info[0]->phone,
			'address_1' => $company_info[0]->address_1,
			'address_2' => $company_info[0]->address_2,
			'reg_address_1' => $company_info[0]->reg_address_1,
			'reg_address_2' => $company_info[0]->reg_address_2,
	...
	public function company_info() {
		...
		'reg_address_1' => $this->input->post('reg_address_1'),
		'reg_address_2' => $this->input->post('reg_address_2'),
	...
	/*  ALL CONSTANTS */	 
	  ...
	  public function payment_method_list()
     {
        ...
        foreach($constant->result() as $r) {
			...
			if($r->payment_method_id==1 || $r->payment_method_id==2){
				$optf = '';
			} else {
				$optf = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".edit_setting_datail" data-field_id="'. $r->payment_method_id . '" data-field_type="payment_method"><span class="fa fa-pencil"></span></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->payment_method_id . '" data-token_type="payment_method"><span class="fa fa-trash"></span></button></span>';
			}
		$data[] = array($optf,
			$comp_name,
			$r->method_name
